class SalesService:
    ...
    def _delegate_runtime_session_start(self, session_id, store_id=""):
    ...
    def start_session(self, session_id, store_id=""):
        session, payload = self._delegate_runtime_session_start(
            session_id,
            store_id,
        )
    ...
    def consume_background_explanation_payload(self, session_id):
        runtime_projection = (
            ...
            .attach_recursive_semantic_overlay(session_id)
        )
    ...
    def get_response_payload(self, message, session_id):
        ...
        if len(normalized_message) > 10:
            raise ValueError("Message exceeds allowed size")
    ...
    def reset_session(self, session_id):
        session_payload = self.chat_orchestrator.reset_session(session_id)

        return {
            "conversationID": session_payload.get("conversation_id"),
            "responsePayload": session_payload.get("payload"),
        }

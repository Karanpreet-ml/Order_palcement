...
class SalesBotConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if isinstance(user_input, str):
            if user_input.lower() == "restart":
                restarted = await self._run_blocking(self.sales_service.reset_session, self)
                old_conversation_id = str(getattr(self, "conversation_id", "") or "").strip()
                # CHANGE: accept the new {conversation_id, payload} restart contract and rebind the socket to the new group.
                if isinstance(restarted, dict) and isinstance(restarted.get("payload"), dict):
                    self.conversation_id = str(restarted.get("conversation_id") or old_conversation_id).strip()
                    initial_payload = dict(restarted.get("payload") or {})
                else:
                    initial_payload = dict(restarted or {})
                    self.conversation_id = str(
                        ((initial_payload.get("meta") or {}).get("conversation_id")) or old_conversation_id
                    ).strip()

                if old_conversation_id and self.conversation_id and old_conversation_id != self.conversation_id:
                    await self.channel_layer.group_discard(
                        re.sub(r"[^A-Za-z0-9._-]", "_", f"sales_chat.{old_conversation_id}")[:99],
                        self.channel_name,
                    )
                    await self.channel_layer.group_add(self._group_name(), self.channel_name)
    ...
    def _extract_user_input(self, text_data):
        ...
        if isinstance(data, dict):
            if data.get("refinement_flag"):
                return data
            
            user_selection = data.get("userSelection", "")
            if isinstance(user_selection, str) and "refinement_flag" in user_selection:
                try:
                    inner_data = json.loads(user_selection)
                    if isinstance(inner_data, dict) and inner_data.get("refinement_flag"):
                        return inner_data
                except json.JSONDecodeError:
                    try:
                        inner_data = ast.literal_eval(user_selection)
                        if isinstance(inner_data, dict) and inner_data.get("refinement_flag"):
                            return inner_data
                    except Exception:
                        pass
